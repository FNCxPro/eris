...
class CommandClient extends Client {
    ...
    * @arg {Function} [commandOptions.preCommand] Function to execute pre command
    ...
    */
    ...
    onMessageCreate(msg) {
        ...
        if((!this.commandOptions.ignoreSelf || msg.author.id !== this.user.id) && (!this.commandOptions.ignoreBots || !msg.author.bot) && (msg.prefix = this.checkPrefix(msg))) {
            ...
            if((command = this.commands[label]) !== undefined || ((command = this.commands[label.toLowerCase()]) !== undefined && command.caseInsensitive)) {
                msg.command = command;
                if(typeof this.commandOptions.preCommand === 'function') {
                    this.commandOptions.preCommand(msg, args, command.caseInsensitive ? label.toLowerCase() : label).then((blacklisted) => {
                        if(blacklisted) return;
                        Promise.resolve(command.process(args, msg)).then((resp) => {
                            this.emit("commandExecuted", command.caseInsensitive ? label.toLowerCase() : label, msg.author, msg, args, command)
                            if(resp != null) {
                                this.createMessage(msg.channel.id, resp).then((newMsg) => {
                                    if(((args && command.options.reactionButtonsArg && args[0] == command.options.reactionButtonsArg) && !(command.options.disableReactionButtonsWithArgs == true && (args && args.length > 0))) && command.reactionButtons) {
                    ...
                    Promise.resolve(command.process(args, msg)).then((resp) => {
                        ...
                        if(resp != null) {
                            this.createMessage(msg.channel.id, resp).then((newMsg) => {
                                if(!(command.options.disableReactionButtonsWithArgs && (args && args.length > 0)) && command.reactionButtons) {
                                    command.reactionButtons.forEach((button) => newMsg.addReaction(button.emoji));
                                    this.activeMessages[newMsg.id] = {
                                        label: command.caseInsensitive ? label.toLowerCase() : label,
                                        invoker: msg.author.id,
    ...
    onMessageReactionEvent(msg, emoji, userID) {
        if(!this.ready || userID === this.user.id || (!msg.embeds && !msg.content)) {
            return;
        }
